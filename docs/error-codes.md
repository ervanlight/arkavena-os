...
| `AI_BUDGET_EXCEEDED` | 422 | Kuota AI bulanan organisasi Anda sudah tercapai. Coba lagi bulan depan atau hubungi admin. | An `ai-scribe` action's `isOverBudget()` check (ADR 0020 SS4) found this organization's `ai_generations` spend this calendar month already at or past `AI_MONTHLY_BUDGET_CAP` -- checked before the Claude API call, not after |
