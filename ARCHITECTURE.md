...
**Dua aturan batas tambahan** (dikunci di `ARCHITECTURE_REVIEW.md` §B, ditegakkan sebagai bagian dari transformasi produk Arkavena OS Fase 12 — IMPLEMENTATION_PLAN.md Fase 1, F25/F26):
...
1. **`client-portal` tidak boleh mengimpor `cash-gate` atau `estimating` secara langsung, sekalipun untuk satu field yang "sudah diterjemahkan".** Setiap data yang sampai ke klien dari kedua modul ini wajib lewat `vw_client_*` view atau lapisan terjemahan eksplisit (Client Project Status, ADR 0026 §2) — tidak pernah lewat pemanggilan fungsi modul secara langsung. Ini menjaga agar data sensitif (margin, rasio kas, cadangan risiko) tidak pernah bisa bocor lewat "tambah satu field kecil" yang dilakukan tergesa-gesa.
   ...
   **Pelanggaran nyata pertama, ditemukan dan diperbaiki (Post-Implementation Review Phase 2, 2026-07-25):** F1 (client-facing proposal acceptance) awalnya mengimpor `@/modules/estimating` langsung dari client-portal — persis skenario yang dikhawatirkan `ARCHITECTURE_REVIEW.md`'s catatan asal aturan ini. Owner memilih mempertahankan aturan ini secara absolut (lihat ADR 0026 §7 item 7) — implementasi diperbaiki, bukan aturannya. **Pola yang benar untuk permukaan keputusan klien** (bukan sekadar tampilan informasi) pada modul yang dilindungi aturan ini: perluas `client_decisions` (tabel milik `client-portal`, disinkron lewat trigger dari tabel sumber — persis `change_order_id`/`fn_change_orders_sync_client_decision`, sekarang juga `proposal_id`/`fn_proposals_sync_client_decision`) untuk pembacaan; untuk satu-satunya tulisan yang tetap harus terjadi (keputusan klien sendiri), gunakan RPC database polos (mis. `fn_client_decide_proposal`) yang dipanggil lewat `supabase.rpc(...)` — pemanggilan prosedur bernama, bukan impor TypeScript lintas modul.
2. **`partner-desk` dan `client-portal` tidak boleh saling berkomunikasi, dalam bentuk apa pun.** Keduanya melayani hubungan eksternal yang berbeda (supplier/subkontraktor vs klien pemilik proyek) — mencampur jalur data keduanya berisiko membocorkan data satu pihak eksternal ke pihak eksternal lain. Belum pernah dilanggar.

**`projects` adalah shared kernel de facto sistem ini** (Phase 3, F27 — dikunci sebagai penamaan eksplisit di `ARCHITECTURE_REVIEW.md` §B, sebelumnya hanya implisit). Per audit terbaru (2026-07-25), `projects` diimpor lewat `index.ts` maupun `server.ts` oleh **9 dari 13 modul lain** (`ai-scribe`, `billing`, `cash-gate`, `crm`, `evidence`, `procurement`, `maintenance-engine`, `quality-gate`, `scope-variation`) — fan-in tertinggi di seluruh sistem, jauh di atas modul lain manapun. Konsekuensi praktis: **perubahan apa pun ke public API `projects` (`index.ts` atau `server.ts`) harus ditinjau seolah itu perubahan `core/`**, bukan modul biasa — sembilan modul lain bergantung pada bentuknya, meski secara struktur ia tetap modul normal (bukan `core/`, karena `projects` tetap membawa konsep domain nyata — proyek, zona, kontrak, milestone, paket kerja — dan `core/` per definisi bebas-domain).
